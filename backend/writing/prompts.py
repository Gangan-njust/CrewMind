...
3. 分析方法：详述算法、模型或统计方法
4. 图表呈现：涉及步骤、模块或流程时建议绘制流程图/结构图；若图表尚未就绪，正文可用「如图 X 所示」预留引用，提醒后续补图，避免通篇纯文字。""",

  "results": """结果章节写作结构：
1. 主要发现：概括核心实验结果
2. 数据呈现：用表格/图表描述关键数据
3. 统计分析：报告显著性检验与置信区间
4. 图表呈现：关键定量对比应配图/表；若图表尚未就绪，正文可用「如图 X 所示 / 见表 X」预留引用，提醒后续补图，避免通篇纯文字。""",
...
以下文献证据仅供术语与表述参考，润色时勿改变原文结构与核心观点，不得编造未出现的数据：
{rag_context}"""
...
FIGURE_CHECK_PROMPT = """你是一位学术论文图表规范审查专家。请检查论文正文中「按学术规范应当配图表、但缺失」的位置，防止论文通篇只有文字。

## 论文各章节正文
{full_text}

## 规则预检命中的候选位置（可参考或纠正，也可补充未覆盖的位置；若无候选则为空）
{candidates}

## 判断标准（结合学术写作规范）
1. 结果章节报告定量数据、方法对比、显著性检验等，通常应以图表呈现关键结果；
2. 方法章节描述步骤、流程、模块结构时，通常应配流程图或结构图；
3. 引言/相关工作若系统梳理方法演进或框架，可配示意图；
4. 正文出现「如图/表 X 所示」的引用时，必须存在对应的图或表；
5. 纯叙述段落、摘要、Abstract、致谢章节无需配图。

## 输出要求（仅输出 JSON）
{{
  "issues": [
    {{
      "section_type": "章节类型（如 results/methods/intro/related_work），与正文中 ## 后括号内一致",
      "anchor_text": "应插入图表的原文锚点句（必须逐字摘自正文，便于前端定位）",
      "chart_type": "柱状图|折线图|散点图|箱线图|直方图|流程图|结构图|表格|示意图",
      "reason": "为何需要配图表",
      "suggestion": "具体建议（含图注要点）",
      "severity": "high|medium|low"
    }}
  ],
  "summary": "总体评价（一句话）"
...
## 约束
- 不虚构任何数据，只指出「在哪里配图、配什么类型的图」；
- 每条 issue 的 anchor_text 必须是正文中真实存在的一句话；
- 同一段落不要重复输出；全文最多输出 10 条；
- 摘要（abstract / abstract_en）与致谢（acknowledgments）不得输出 issue。"""
...
CITATION_RECOMMEND_RAG_PROMPT = """你是一位文献引用推荐专家。用户选中了以下文本，请从检索到的文献证据片段中推荐最相关的引用。
...
要求：
- 仅推荐证据片段中真实出现的内容，不得虚构
- 按相关度降序排列，最多推荐 5 条
- chunk_id、excerpt、section_key 必须与证据片段一致"""
...
KEYWORDS_GENERATION_PROMPT = """你是一位中文学术论文写作专家。请根据以下中文摘要，提取 3–5 个最能代表论文核心内容的中文关键词。
...
要求：
1. 关键词数量必须为 3–5 个
2. 每个关键词 2–8 个汉字，学术规范，避免过于宽泛（如「研究」「分析」单独作为关键词）
3. 关键词应覆盖研究对象、方法、应用场景等核心要素
4. 输出 JSON 格式：
{{
...
ABSTRACT_TRANSLATION_PROMPT = """你是一位学术英译专家。请将以下中文摘要翻译为规范的英文 Abstract。
...
要求：
1. 使用正式学术英语，时态与语态符合英文论文 Abstract 规范
2. 覆盖背景、方法、主要结果与结论
3. 约 250–300 词，输出为连贯段落
4. 不要添加 Keywords 行或任何标题
5. 直接输出英文 Abstract 正文，不要解释"""


KEYWORDS_TRANSLATION_PROMPT = """你是一位学术英译专家。请将以下中文关键词翻译为规范的英文 Keywords。

中文关键词：{keywords}

要求：
1. 每个中文关键词对应一个英文关键词或短语
2. 使用学术领域常用英文表达
3. 保持与中文关键词数量一致
4. 输出 JSON 格式：
{{
