...
LITERATURE_REVIEW_CONTEXT_PROMPT = """以下为用户文献库中选定文献及其已有 AI 分析结果。请基于这些文献撰写一篇符合学术规范的系统性文献综述（literature review），并将本提示作为**核心素材与引用清单**。

## 综述主题
{topic}

## 写作要求
- 综述须围绕主题组织，严禁逐篇罗列摘要；应按照主题维度或发展脉络对文献进行归纳、比较与评述。
- 正文引用以 [编号] 标注，编号须与下方「选定文献与参考文献」一致；文末「参考文献」章节直接采用下列编号与著录信息，可补充网络检索文献但须另行编号并标注来源。
- 指出文献之间的承继、分歧、互补关系，并识别当前研究的不足、研究空白与未来发展方向。
- 网络检索仅作背景补充，须在「文献数据库说明」中区分用户文献库与网络来源。
- 不得虚构用户文献库中不存在的文献、结论或数据。
...
## 建议的综述结构（可调整细化）
1. 引言（研究背景与意义、综述目的与范围界定）
2. 文献检索与筛选说明
3. 国内外研究现状（按主题/方法/发展阶段分类综述，含述评）
4. 主要研究方法与技术对比
5. 现有研究存在的主要问题与争论
6. 研究空白与未来展望
7. 结论
